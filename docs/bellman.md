...
>
> 관련 문서: `docs/lbo.md` (Bellman → HJB → diffusion → Laplace–Beltrami 및 설계공간 LBO 정규화)

### 초록
강화학습에서 벨만 방정식과 동적계획법(DP)은 **고정된** 마르코프 결정과정(MDP) 위에서 성립한다. 
반면 “게임 설계”는 맵, 유닛, 팩션 규칙 등 **MDP 자체를 바꾸는 상위 수준의 최적화 문제**이다.

본 장은 (i) 플레이 문제와 설계 문제를 수학적으로 분리하여 정식화하고, (ii) 왜 벨만/DP가 설계 문제의 바깥 레벨에는 적용되지 않는지를 원리 수준에서 보이며, (iii) 설계 문제를 확률적 black-box 최적화 및 bilevel 최적화로 정리한다.
